 ...
 *  * 18:02:08     Source header: sokol_gl.h     Module: sokol.gl     Do not edit
...
/++
 * sgl_logger_t      Used in sgl_desc_t to provide a custom logging and error
 *  * reporting     callback to sokol-gl.
 +/
...
/++
 * sgl_error_t      Errors are reset each frame after calling sgl_draw(),     get
 *  * the last error code with sgl_error()
 +/
...
/++
 * sgl_context_desc_t      Describes the initialization parameters of a rendering
 *  * context.     Creating additional contexts is useful if you want to render
...
/++
 * sgl_allocator_t      Used in sgl_desc_t to provide custom memory-alloc and -free
 *  * functions     to sokol_gl.h. If memory management should be overridden, both
 *  * the     alloc and free function must be provided (e.g. it's not valid to
 ...
 *  * (valid inside or outside begin/end)
 +/
