 ...
 *  * 18:02:08     Source header: sokol_app.h     Module: sokol.app     Do not edit
...
/++
 * sapp_event_type      The type of event that's passed to the event handler
 *  * callback     in the sapp_event.type field. These are not just "traditional"
 *  * input events, but also notify the application about state changes     or
 *  * other user-invoked actions.
 +/
enum EventType {
    Invalid,
    Key_down,
...
/++
 * sapp_keycode      The 'virtual keycode' of a KEY_DOWN or KEY_UP event in the
 *  * struct field sapp_event.key_code.      Note that the keycode values are
 *  * identical with GLFW.
 +/
enum Keycode {
    Invalid = 0,
 ...
 *  * NOTE: the values must remain in sync with the corresponding     Android SDK
 *  * type, so don't change those.      See https://developer.android.com/reference
 *  * /android/view/MotionEvent#TOOL_TYPE_UNKNOWN
 +/
...
/++
 * sapp_touchpoint      Describes a single touchpoint in a multitouch event
 *  * (TOUCHES_BEGAN,     TOUCHES_MOVED, TOUCHES_ENDED).      Touch points are
 *  * stored in the nested array sapp_event.touches[],     and the number of
 *  * touches is stored in sapp_event.num_touches.
 +/
...
/++
 * sapp_mousebutton      The currently pressed mouse button in the events
 *  * MOUSE_DOWN     and MOUSE_UP, stored in the struct field
 *  * sapp_event.mouse_button.
 +/
enum Mousebutton {
    Left = 0,
    Right = 1,
...
/++
 * sapp_event      This is an all-in-one event struct passed to the event handler
 *  * user callback function. Note that it depends on the event     type what
 *  * struct fields actually contain useful values, so you     should first check
 *  * the event type before reading other struct     fields.
 +/
...
/++
 * sg_range      A general pointer/size-pair struct and constructor macros for
 *  * passing binary blobs     into sokol_app.h.
 +/
...
/++
 * sapp_image_desc      This is used to describe image data to sokol_app.h (at
 *  * first, window     icons, later maybe cursor images).      Note that the
 *  * actual image pixel format depends on the use case:      - window icon pixels
 *  * are RGBA8
 +/
...
/++
 * sapp_icon_desc      An icon description structure for use in sapp_desc.icon and
 *  * sapp_set_icon().      When setting a custom image, the application can
 *  * provide a number of     candidates differing in size, and sokol_app.h will
 *  * pick the image(s)     closest to the size expected by the platform's window
 *  * system.      To set sokol-app's default icon, set .sokol_default to true.
 *  * Otherwise provide candidate images of different sizes in the     images[]
 *  * array.      If both the sokol_default flag is set to true, any image
 *  * candidates     will be ignored and the sokol_app.h default icon will be set.
 +/
...
/++
 * sapp_allocator      Used in sapp_desc to provide custom memory-alloc and -free
 *  * functions     to sokol_app.h. If memory management should be overridden, both
...
/++
 * sapp_logger      Used in sapp_desc to provide a logging function. Please be
 *  * aware that     without logging function, sokol-app will be completely silent,
 *  * e.g. it will     not report errors or warnings. For maximum error verbosity,
 *  * compile in     debug mode (e.g. NDEBUG *not* defined) and install a logger
...
/++
 * sokol-app initialization options, used as return value of sokol_main()     or
 *  * sapp_run() argument.
 +/
...
/++
 * HTML5 specific: request and response structs for    asynchronously loading
...
/++
 * sapp_mouse_cursor      Predefined cursor image definitions, set with
...
/++
 * call from inside event callback to consume the current event (don't forward to
 *  * platform)
 +/
...
/++
 * Metal: get bridged pointer to MTKView's msaa-color-texture of type MTLTexture
 *  * (may be null)
 +/
...
/++
 * D3D11: get pointer ID3D11RenderTargetView object for msaa-resolve (may return
 *  * null)
 +/
