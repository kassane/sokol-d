 ...
 *  * 18:02:09     Source header: sokol_imgui.h     Module: sokol.imgui     Do not
...
/++
 * simgui_allocator_t      Used in simgui_desc_t to provide custom memory-alloc and
 *  * -free functions     to sokol_imgui.h. If memory management should be
...
/++
 * simgui_logger      Used in simgui_desc_t to provide a logging function. Please
 *  * be aware     that without logging function, sokol-imgui will be completely
 *  * silent, e.g. it will not report errors, warnings and     validation layer
 *  * messages. For maximum error verbosity,     compile in debug mode (e.g. NDEBUG
 *  * *not* defined) and install     a logger (for instance the standard logging
 *  * function from sokol_log.h).
 +/
