 ...
 *  * 18:02:09     Source header: sokol_debugtext.h     Module: sokol.debugtext
...
/++
 * sdtx_logger_t      Used in sdtx_desc_t to provide a custom logging and error
 *  * reporting     callback to sokol-debugtext.
 +/
...
/++
 * sdtx_range is a pointer-size-pair struct used to pass memory     blobs into
 *  * sokol-debugtext. When initialized from a value type     (array or struct),
 *  * use the SDTX_RANGE() macro to build     an sdtx_range struct.
 +/
...
/++
 * sdtx_context_desc_t      Describes the initialization parameters of a rendering
 *  * context. Creating     additional rendering contexts is useful if you want to
 *  * render in     different sokol-gfx rendering passes, or when rendering several
 *  * layers     of text.
 +/
...
/++
 * sdtx_allocator_t      Used in sdtx_desc_t to provide custom memory-alloc and
 *  * -free functions     to sokol_debugtext.h. If memory management should be
 *  * overridden, both the     alloc_fn and free_fn function must be provided (e.g.
 *  * it's not valid to     override one function but not the other).
 +/
...
/++
 * sdtx_desc_t      Describes the sokol-debugtext API initialization parameters.
 *  * Passed     to the sdtx_setup() function.      NOTE: to populate the fonts
 *  * item array with builtin fonts, use any     of the following functions:
 *  * sdtx_font_kc853()         sdtx_font_kc854()         sdtx_font_z1013()
 *  * sdtx_font_cpc()         sdtx_font_c64()         sdtx_font_oric()
 +/
