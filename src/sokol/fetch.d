 ...
 *  * 18:02:09     Source header: sokol_fetch.h     Module: sokol.fetch     Do not
 *  * edit manually; regenerate using gen_d.py.
 +/
module sokol.fetch;
...
/++
 * sfetch_logger_t      Used in sfetch_desc_t to provide a custom logging and error
 *  * reporting     callback to sokol-fetch.
 +/
...
/++
 * sfetch_range_t      A pointer-size pair struct to pass memory ranges into and
 *  * out of sokol-fetch.     When initialized from a value type (array or struct)
 *  * you can use the     SFETCH_RANGE() helper macro to build an sfetch_range_t
 *  * struct.
 +/
...
/++
 * sfetch_allocator_t      Used in sfetch_desc_t to provide custom memory-alloc and
 *  * -free functions     to sokol_fetch.h. If memory management should be
 *  * overridden, both the     alloc and free function must be provided (e.g. it's
 *  * not valid to     override one function but not the other).
 +/
...
/++
 * return the max userdata size in number of bytes (SFETCH_MAX_USERDATA_UINT64 *
 *  * sizeof(uint64_t))
 +/
...
/++
 * clear the 'buffer binding' of a request, returns previous buffer pointer (can be
 *  * 0), must be called from response callback
 +/
...
/++
 * cancel a request that's in flight (will call response callback with .cancelled +
 *  * .finished)
 +/
